...
def train(model, optimizer, criterion, batched_graph, batched_features, batched_labels):
    model.train()
...
def evaluate_ppi(model, data_loader, criterion):
    model.eval()
...
def train_loop_ppi(model, optimizer, criterion, train_loader, val_loader, epochs):
    ...
    for epoch in range(epochs):
        ...
        for batched_graph, batched_features, batched_labels in train_loader:
            train_loss, train_acc = train(model, optimizer, criterion, batched_graph, batched_features, batched_labels)
        ...
        val_acc, val_loss = evaluate_ppi(model, val_loader, criterion)
...
#     model.train()
#     logits = model(batched_graph, batched_features)  # Forward pass
#     loss = criterion(logits, batched_labels)  # Compute loss
#     optimizer.zero_grad()
#     loss.backward()
#     optimizer.step()

#     predicted = (torch.sigmoid(logits) > 0.5).float().cpu().numpy()  # Convert predictions to binary
#     labels = batched_labels.cpu().numpy()  # Convert labels to numpy for F1 calculation
...
#         model.train()
#         for batched_graph, batched_features, batched_labels in train_loader:
#             train_loss, train_f1 = train(model, optimizer, criterion, batched_graph, batched_features, batched_labels)
#             total_train_loss += train_loss
...
#         val_f1, val_loss = evaluate_ppi(model, val_loader, criterion)
#         val_losses.append(val_loss)
