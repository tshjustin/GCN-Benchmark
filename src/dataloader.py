...
def train_test_val_split(labels, num_classes, config):
    """Split dataset into train, validation, and test sets, proportional to class sizes to prevent class imbalance."""
    classes = [i for i in range(num_classes)]
    train_set = []
    validation_set = []
    test_set = []
    ...
    for class_label in classes:
        ...
        # Calculate the number of samples for train, val, test for each class
        num_train = int(np.floor(num_class_samples * config.train_proportion))
        num_val = int(np.floor(num_class_samples * config.validation_proportion))
        num_test = num_class_samples - num_train - num_val

        # Assign indices to each set
        class_train_set = target_indices[:num_train]
        class_val_set = target_indices[num_train:num_train + num_val]
        class_test_set = target_indices[num_train + num_val:]

        # Add to the global sets
        train_set += [ind[0] for ind in class_train_set]
        validation_set += [ind[0] for ind in class_val_set]
        test_set += [ind[0] for ind in class_test_set]
...
def load_squirrel_data(config):
    print("Loading Squirrel Dataset")

    raw_data = np.genfromtxt(config.nodes_path, dtype=str, skip_header=1, delimiter="\t")
    node_id = raw_data[:, 0].astype('int32')  # Node IDs (already ordered)
    node_labels = raw_data[:, -1] 
    
    labels_enumerated = map_labels(node_labels) 


    feature_strings = raw_data[:, 1:-1]  # Extract features as strings
    node_features = np.array([list(map(float, row[0].split(','))) for row in feature_strings], dtype="float32")
    node_features_sparse = sparse.csr_matrix(node_features)

    raw_edges_data = np.genfromtxt(config.edges_path, dtype="int32", skip_header=1, delimiter="\t")
    edges_ordered = raw_edges_data  # Since nodes are already ordered, no reordering  needed


    adj_normalized = create_adjacency_matrix(edges_ordered, labels_enumerated)
    features = torch.FloatTensor(node_features_sparse.toarray())
    labels = torch.LongTensor(labels_enumerated)
    adj = covert_to_pytorch_format(adj_normalized)

    print("Loaded Squirrel Dataset")
